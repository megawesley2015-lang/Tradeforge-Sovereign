...
## Dívida técnica registrada (Next.js 16)

### 1) `middleware.ts` → `proxy.ts`
Next.js 16 deprecou a convenção `middleware` em favor de `proxy`.
Warning atual no terminal:
> The "middleware" file convention is deprecated. Please use "proxy" instead.

**Por que não migramos ainda:** ainda funciona em modo deprecation; migrar agora exige reescrever o file convention e validar todos os redirects de auth. Fica para depois do MVP entrar em produção.

**Quando migrar:** após o primeiro deploy estável na Vercel, antes da próxima major (Next 17).

**Como migrar (resumo):**
1. Renomear `middleware.ts` → `proxy.ts` na raiz.
2. Renomear `export async function middleware` → `export async function proxy`.
3. Manter o mesmo `config.matcher`.
4. Smoke test: login, callback PKCE, redirect de `/trading/*` sem sessão.
Doc oficial: https://nextjs.org/docs/messages/middleware-to-proxy

### 2) Filesystem lento detectado
Warning:
> Slow filesystem detected. The benchmark took 422ms.

**Causa provável:** `.next/dev` no disco do usuário com indexador do Windows + antivírus.
**Mitigação imediata (opcional):** adicionar `C:\Users\wesley\Documents\tradeforge-sovereign\.next` à lista de exclusão do Windows Defender.
**Mitigação definitiva (futuro):** mover o projeto para fora do path `Documents` (que tem OneDrive sync ativo) — sugestão: `C:\dev\tradeforge-sovereign`.
...
- Context root: `c:\Users\wesley\Documents\tradeforge-sovereign`
...
```bash
cd c:\Users\wesley\Documents\tradeforge-sovereign
