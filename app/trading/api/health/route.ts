...
// Retorna metricas de saude do bot para o Dashboard de Saude.
...
export async function GET() {
  ...
  // 'unknown' = Vercel bloqueou o ping -- nao conta como falha real
  const anyDown  = Object.values(providers).some(s => s === 'down');
  const allGone  = Object.values(providers).every(s => s === 'down' || s === 'unknown');
...
// ── Provider ping ──────────────────────────────────────────────
// NOTA: Vercel Hobby pode bloquear conexoes de saida para algumas
// exchanges. Nesse caso retorna 'unknown' (nao 'down') para evitar
// falso-positivo no status geral do sistema.

type ProviderStatus = 'up' | 'slow' | 'down' | 'unknown';
  ...
  await Promise.allSettled(
    checks.map(async ({ name, url }) => {
      ...
      try {
        ...
        results[name] = isTimeout ? 'slow' : 'unknown';
      }
    })
  );
